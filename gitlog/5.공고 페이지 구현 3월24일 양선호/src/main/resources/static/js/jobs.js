document.addEventListener("DOMContentLoaded", () => {
    ...
    function formatEmploymentType(text) {
        if (!text) return "-";
        return text.replace(/\n/g, " / ");
    }
    ...
    function renderJobs(jobs) {
        ...
        if (!jobs || jobs.length === 0) {
            jobList.innerHTML = `<div class="card" style="padding:16px;">해당 카테고리의 공고가 없어.</div>`;
            return;
        }
                    ...
                    <div class="job-main-row">
                        <div class="job-title-area">
                            ...
                            <div class="job-link-row">
                                <a href="${job.postingUrl ?? '#'}" target="_blank" class="job-link-btn">공고 바로가기</a>
                            </div>
                        </div>
    ...
    async function loadJobs(categoryId, page = 1) {
        try {
