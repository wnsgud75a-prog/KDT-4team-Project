...
// [수정] 실제 스프링 빈으로 등록되도록 @Service를 주석 밖으로 분리
@Service
public class UserService {

    private final UserRepository userRepository;

    // BCryptPasswordEncoder : 회원가입과 비밀번호 변경 시 비밀번호를 암호화하기 위한 객체
    ...
    // existsByLoginId : 로그인 아이디 중복 여부를 확인
    public boolean existsByLoginId(String loginId) {
        return userRepository.existsByLoginId(loginId);
    }

    // 비밀번호를 암호화한 뒤 회원 정보 저장
    // [수정] 저장 메서드에 트랜잭션이 실제 적용되도록 분리
    @Transactional
    ...
    // [수정] 회원가입 저장 흐름에 트랜잭션이 실제 적용되도록 분리
    @Transactional
    ...
    // [수정] 프로필 수정 메서드에 트랜잭션이 실제 적용되도록 분리
    @Transactional
    public User updateProfile(UserProfileRequest request) {
        ...
        // [수정] 새 비밀번호가 들어오면 암호화해서 PW 컬럼도 함께 변경
    ...
    // [수정] 리스트 형태의 기술스택 이름을 DB에 저장할 문자열로 변환
